...
export function GalleryManager({ initialImages, adminUid }: GalleryManagerProps) {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check sizes and compile valid files
    ...
    try {
      ...
      const uploadPromises = validFiles.map(async (file) => {
        const fileRef = ref(storage, `gallery/${Date.now()}_${Math.random().toString(36).substring(7)}_${file.name}`);
      ...
      alert("Lỗi tải một số ảnh lên. Vui lòng thử lại. Chi tiết: " + error.message);
    } finally {
      if (fileInputRef.current) fileInputRef.current.value = "";
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
      ...
      <div className="flex flex-col md:flex-row gap-3 mb-8">
        ...
        <div className="flex gap-2">
          <Button variant="outline" onClick={handleAddUrl} className="gap-2 shrink-0">
